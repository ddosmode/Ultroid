...
async def filestoreplg(event):
    ...
    # разрешить сохранение как сообщений, так и медиа.
...
async def _(event):
    ...
    if not match:
        return await event.eor("Укажите ссылку на сохранённый файл для удаления.", time=5)
    match = match.split("?start=")
    botusername = match[0].split("/")[-1]
    if botusername != asst.me.username:
        return await event.eor(
            "Сообщение/медиа по предоставленной ссылке не было сохранено этим ботом.", time=5
        )
    msg_id = get_stored_msg(match[1])
    if not msg_id:
        return await event.eor(
            "Сообщение/медиа по предоставленной ссылке уже было удалено.", time=5
        )
    del_stored(match[1])
    await ultroid_bot.delete_messages(udB.get_key("LOG_CHANNEL"), int(msg_id))
    await event.eor("__Удалено__")
...
async def file_short(event):
    ...
    if not res:
        title = "У вас нет сохранённых файлов :("
        text = f"{title}\n\nПрочтите `{HNDLR}help fileshare`, чтобы узнать, как сохранять."
        return await event.answer([await event.builder.article(title=title, text=text)])
    await event.answer(res, switch_pm="• Хранилище файлов •", switch_pm_param="start")
