...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                f"git clone -q -b {Repo().active_branch} https://github.com/TeamTeleFriend/TeleFriendAddons.git addons",
                shell=True,
            )
        else:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                "git clone -q https://github.com/TeamTeleFriend/TeleFriendAddons.git addons",
                shell=True,
            )
    ...
    if vcbot and (vcClient and not vcClient.me.bot):
        try:
            ...
            if not os.path.exists("vcbot"):
                subprocess.run(
                    "git clone https://github.com/TeamTeleFriend/VcBot vcbot", shell=True
                )
            try:
